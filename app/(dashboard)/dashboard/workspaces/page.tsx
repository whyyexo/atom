...
export default function WorkspacesPage() {
  ...
  return (
    ...
    >
      ...
      <section className="flex flex-col gap-4 rounded-3xl border border-border/60 bg-card/95 p-5 shadow-md shadow-primary/5 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-wrap items-center gap-3">
          ...
          {filters.map((filter) => (
            <Button
              ...
              className={filter === selectedFilter ? "rounded-full bg-primary px-3 text-primary-foreground" : "rounded-full px-3"}
        ...
        <div className="flex w-full items-center gap-2 rounded-2xl border border-border/70 bg-muted/30 px-3 py-2 md:w-72">
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        {filteredWorkspaces.length === 0 ? (
          <Card className="rounded-3xl border-border/60 bg-muted/20 p-10 text-center text-sm text-muted-foreground">
          ...
          filteredWorkspaces.map((workspace, index) => (
            <motion.div
              ...
              className="group flex flex-col rounded-3xl border border-border/60 bg-card/95 p-5 shadow-sm transition hover:-translate-y-1 hover:border-primary/60 hover:shadow-lg hover:shadow-primary/10"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/15 text-sm font-semibold text-primary">
                    {workspace.name
                      .split(" ")
                      .map((word) => word[0])
                      .join("")
              ...
              <div className="mt-5 flex items-center justify-between text-xs text-muted-foreground">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" size="icon" className="rounded-full border border-border/60 text-muted-foreground hover:text-foreground">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <Card className="rounded-3xl border-border/60 bg-muted/20 p-6">
          <CardHeader className="px-0 pt-0">
            <CardTitle className="text-base text-foreground">Workspace capacity</CardTitle>
            <CardDescription>Monitor seats used vs. available across your organization.</CardDescription>
          </CardHeader>
          <CardContent className="px-0">
            <div className="flex items-center justify-between text-sm text-muted-foreground">
              <span>Seats occupied</span>
              <span className="text-foreground">58 / 75</span>
            </div>
        ...
        <Card className="rounded-3xl border-border/60 bg-muted/20 p-6">
          <CardHeader className="px-0 pt-0">
            <CardTitle className="text-base text-foreground">Active integrations</CardTitle>
            <CardDescription>Connected data sources driving context for your agents.</CardDescription>
          </CardHeader>
          <CardContent className="px-0 space-y-2">
            {["Linear", "Notion", "Postgres analytics", "Slack"].map((integration) => (
              ...
              >
                {integration}
                <Badge variant="secondary" className="rounded-full border border-border/60 text-xs">
                  Connected
                </Badge>
              </div>
            ))}
        ...
        <Card className="rounded-3xl border-border/60 bg-muted/20 p-6">
          <CardHeader className="px-0 pt-0">
            <CardTitle className="text-base text-foreground">Workspace blueprints</CardTitle>
            <CardDescription>Start faster with curated templates for common org functions.</CardDescription>
          </CardHeader>
          <CardContent className="px-0 space-y-3">
            {["Sales acceleration", "Incident command", "R&D insights"].map((blueprint) => (
              <div
                key={blueprint}
                className="flex items-center justify-between rounded-xl border border-border/50 bg-background/80 px-3 py-2 text-sm transition hover:border-primary/60 hover:text-foreground"
              >
                {blueprint}
      ...
      <section className="rounded-3xl border border-dashed border-border/70 bg-muted/10 p-6 text-sm text-muted-foreground">
