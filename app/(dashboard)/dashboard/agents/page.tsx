...
const statusTone: Record<string, string> = {
  ...
  Pilot: "bg-amber-500/15 text-amber-300",
  "In review": "bg-sky-500/15 text-sky-300",
};
      ...
      <Card className="rounded-3xl border-border/60 bg-card/95 shadow-lg shadow-primary/5">
        <CardHeader className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex w-full flex-col gap-3 md:w-auto md:flex-row md:items-center">
            <div className="relative flex items-center gap-2 rounded-2xl border border-border/70 bg-muted/30 px-3 py-2">
            ...
            <Badge variant="secondary" className="h-8 rounded-full border border-border/70 px-3">
        ...
        <CardContent className="space-y-4">
          <div className="hidden grid-cols-12 items-center gap-4 rounded-2xl border border-border/60 bg-muted/20 px-4 py-3 text-xs font-medium text-muted-foreground md:grid">
              ...
              >
                <div className="grid grid-cols-1 gap-4 md:grid-cols-12 md:items-center">
                  ...
                  <div className="flex items-center justify-between md:col-span-2 md:block">
                    ...
                    <Badge variant="outline" className="mt-1 rounded-full border-border/60 text-xs text-muted-foreground">
                      {agent.model}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between md:col-span-2 md:block">
                    <span className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Latency</span>
                    <p className="text-sm text-foreground md:mt-1">{agent.latency}</p>
                  </div>
                  <div className="flex items-center justify-between md:col-span-2 md:block">
                    <span className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Impact</span>
                    <p className="text-sm text-emerald-400 md:mt-1">{agent.retention}</p>
                  </div>
                  <div className="flex items-center justify-end gap-2 md:col-span-1">
                    <Badge
                      className={cn("rounded-full px-3 text-xs font-medium", statusTone[agent.status] ?? "bg-muted")}
                    >
                      {agent.status}
                    </Badge>
